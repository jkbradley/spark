...
private[spark] class EntropyCalculator(stats: Array[Double]) extends ImpurityCalculator(stats) {
  ...
  override def copy: EntropyCalculator = new EntropyCalculator(stats.clone())

  override def calculate(): Double = Entropy.calculate(stats, stats.sum)

  override def count: Double = stats.sum
