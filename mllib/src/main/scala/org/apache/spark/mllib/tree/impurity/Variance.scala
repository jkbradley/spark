...
private[spark] class VarianceCalculator(stats: Array[Double]) extends ImpurityCalculator(stats) {
  ...
  override def copy: VarianceCalculator = new VarianceCalculator(stats.clone())

  override def calculate(): Double = Variance.calculate(stats(0), stats(1), stats(2))

  override def count: Double = stats(0)
