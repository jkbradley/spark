...
object LDATiming {
  ...
  private def run(params: Params) {
    ...
    for (corpusSize <- params.corpusSizes) {
      for (vocabSize <- params.vocabSizes) {
        ...
        val (corpus, vocabArray, selectedTokenCount) =
        ...
        for (k <- params.ks) {
          ...
          val topics = ldaModel.describeTopics(maxTermsPerTopic = 10).map {
            case (termIndices: Array[Int], termWeights: Array[Double]) =>
              termIndices.zip(termWeights).map { case (weight, term) =>
                (weight, vocabArray(term.toInt))
              }
          }
          println(s"$k topics:")
  ...
  private def preprocess(
    ...
    val textRDD_tmp: RDD[String] = sc.textFile(paths.mkString(","))
    ...
    val documents = tokenized.map { case (id, tokens) =>
      ...
      val sv = Vectors.sparse(vocab.size, indices, values)
      (id, sv)
    }
